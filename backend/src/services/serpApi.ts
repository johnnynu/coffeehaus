...
export class SerpApiService {
  ...
  async searchGooglePlaces(
    ...
    try {
      ...
      if (location) {
        params.append(
          "ll",
          `@${location.lat},${location.lng},${Math.floor(radius / 1000)}z`
        );
      }

      const response = await axios.get<SerpApiGooglePlacesResponse>(
        this.baseUrl,
        { params }
      );
  ...
  async getGooglePlaceDetails(
    placeId: string
  ): Promise<Omit<CoffeeShop, "id" | "created_at" | "updated_at"> | null> {
    try {
      const params = new URLSearchParams({
        engine: "google_maps",
        place_id: placeId,
      ...
      return this.convertGooglePlaceDetailsToCustomFormat(
  ...
  private convertGooglePlaceToCustomFormat(
    place: GooglePlaceResult
  ): Omit<CoffeeShop, "id" | "created_at" | "updated_at"> | null {
    try {
      // Parse address components
      const addressParts = place.address.split(", ");
      const zipMatch = addressParts[addressParts.length - 1]?.match(
        /([A-Z]{2})\s+(\d{5}(-\d{4})?)/
      );
      ...
      const hours = place.hours
        ? this.convertSerpApiHours(place.hours)
        : undefined;

      // Convert price level
      const priceLevel = place.price
        ? this.parsePriceLevel(place.price)
        : undefined;
  ...
  private convertGooglePlaceDetailsToCustomFormat(
    ...
    try {
      ...
      console.error(
        "Error converting Google Place details to CoffeeShop:",
        error
      );
      return null;
    }
  ...
  private convertSerpApiHours(
    hours: { [key: string]: string } | string
  ):
    | { [key: string]: { open: string; close: string; is_closed: boolean } }
    | undefined {
    // If hours is a string, parse it differently
    if (typeof hours === "string") {
      return this.parseHoursString(hours);
    }

    // fallback hours parsing
  ...
  private parseHoursString(
    ...
    if (
      lowerHours.includes("open 24 hours") ||
    ...
    if (lowerHours.includes("closed")) {
      ...
      const openMatch = hoursString.match(
        /opens\s+(\d{1,2}:?\d{0,2})\s*(am|pm)/i
      );
      if (openMatch) {
        ...
        for (const day of allDays) {
          convertedHours[day] = {
            open:
              openMatch[1] +
              (openMatch[1].includes(":") ? "" : ":00") +
              " " +
              openMatch[2].toUpperCase(),
    ...
    const openCloseMatch = hoursString.match(
      /closes?\s+(\d{1,2}:?\d{0,2})\s*(am|pm)/i
    );
    if (openCloseMatch && lowerHours.includes("open")) {
      ...
      for (const day of allDays) {
        convertedHours[day] = {
          ...
          close:
            openCloseMatch[1] +
            (openCloseMatch[1].includes(":") ? "" : ":00") +
            " " +
            openCloseMatch[2].toUpperCase(),
          is_closed: false,
        };
      }
      return convertedHours;
